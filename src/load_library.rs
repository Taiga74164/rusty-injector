...
pub unsafe fn load_library_dll(h_proc: HANDLE, dll_path: &CString) -> bool {
    ...
    if h_kernel.is_null() {
        println!("Error getting kernel32.dll handle");
        return false;
    }

    let load_lib = GetProcAddress(h_kernel, "LoadLibraryA\0".as_ptr() as *const i8);
    if load_lib.is_null() {
        println!("Error getting LoadLibraryA address");
        return false;
    }
